...
from src.drawing_utils import chess_square_to_screen_square, screen_square_to_chess_square, square_to_position
...
from src.utils import algebraic

def squares_attacked(game: ChessBoard, selected_square: Tuple[int]):
    squares = set()
    selected_chess_square = screen_square_to_chess_square(selected_square)
    for move in game.legal_moves[game.turn]:
...
def draw_notepad(screen, board: ChessBoard, notes_background_image, selected_square, evaluation_function):
    ...
    for row in range(ROWS):
        for col in range(COLS):
            ...
            if highlight_last_move and board.history:
                ...
                capture = bool(last_move_record.captured_piece)
            ...
            if highlight_attack and selected_square is not None and (row,col) in squares_attacked(board,selected_square):
                ...
                if board.grid[chess_row][chess_col]*board.turn < 0:
                    highlight_color = HIGHLIGHT_CAPTURE
                else:
                    highlight_color = HIGHLIGHT
                color = tuple([sum(x)//2 for x in zip(color,highlight_color)])
...
def animate_holding_piece(screen, board, piece_images, selected_square):
